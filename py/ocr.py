...
filenames = [f.split(".")[0] for f in os.listdir(pdfs) if f.endswith(".pdf")]
...
def convert_pdf_to_images(pdf_path, filename):
    ...
    for index, image in enumerate(images):
        ...
        print(f"{pdf_path} successfully converted to png")
...
def fetchTableLineIDs (txtFile):
    ...
    for line in txtFile.split("\n"):
        ...
        if len(line) != 0:
            ...
            elif re.match(r"^FD-|BIO-|BIO |FD |BIO", line) and len(line) > 5:
            ...
            elif re.search(r"for Branch$", line) != None:
...
def checkBrokenLines (txtFile, brokenIDs, actualDescriptionIDs, possibleDescriptionIDs):
    ...
    for line in txtFile.split("\n"):
        ...
        if len(line) != 0:
            ...
            elif re.search(r"00 PCS$", line) != None:
                quantities.append(line)
    ...
    # print(len(actualDescriptions))   
    # print(len(possibleDescriptions))     
...
for name in txtfilenames:
    ...
    if allowOCR == True:
        ...
        for index, row in customerNames.iterrows():
            ...
            # if row["LPO NAME"] == customerBranch:
                print(row)
        ...
        outputTable = pd.DataFrame({"LPO CUSTOMER NAME": (customerBranch), "BOOK 365 NAME": book_365_name, "WAREHOUSE": "FGStore", "CONTRACT NUMBER": contractNumber, "CONTRACT DATE": contractDate, "DOCUMENT DATE":contractDate, "DESC OCR": orderItemDescriptions, "BARCODES": productBarcodes, "QUANTITY": orderItemQuantities, "TRAYS": "trays"})
        outputTable["BARCODES"] = outputTable["BARCODES"].astype("string")

        orderedColumnNames = ["LPO CUSTOMER NAME", "BOOK 365 NAME", "DOCUMENT DATE", "WAREHOUSE", "CONTRACT NUMBER", "CONTRACT DATE", "SKU", "QUANTITY", "BARCODES", "DESC OCR", "BOOK 365 DESC", "Pieces", "TRAYS"]
        ...
        for index in finalTable.index:
            if finalTable["Pieces"][index] != None:
                finalTable["TRAYS"][index] = finalTable["QUANTITY"][index] / finalTable["Pieces"][index]
            else:
                finalTable["TRAYS"] = "Not converted"

        finalTable.to_excel(xlsxs + "/" + name + ".xlsx")
